...
class Extension(SK_Extension):
    ...
    def __init__(self, *args, **kwargs):
        ...
        print(f"init with args: {args} and kwargs: {kwargs}")
    ...
    def event_start(self, *args):
        ...
        while self.is_running: 
            ...
            self.debug(f" running for {time.time() - start_time} seconds", debug_level=0)
            self.debug(f"Try 'ext cmd [number]' to change the sleep time is ms", debug_level=0)
    ...
    def event_receive_commands(self, commands:list[str]):
        for command in commands:
            try:
                self.sleep_time = int(command)
            except Exception as e:
                self.debug(f"Error processing command: {command}", debug_level=0, type=TYPE_ERROR)
        return 
